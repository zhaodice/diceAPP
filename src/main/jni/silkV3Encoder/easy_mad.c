...
//    return MAD_FLOW_CONTINUE;
    ...
    retval = read((int) handle->fin, mp3_buf + keep, MP3_BUF_SIZE - keep);
    if (retval < 0) {
        /* Read error. */
                     ...
                     struct mad_pcm *pcm)
{
    ...
    while (nsamples--) {
        signed int sample;
        sample = scale(*left_ch++);
        *(wptr++) = sample<<8;//((sample >> 0) & 0xff);
        *(wptr++) = sample>>8;//((sample >> 8) & 0xff);
        /*if (nchannels == 2) {
            sample = scale(*right_ch++);
            *(a) |= sample<<8;//((sample >> 0) & 0xff);
            *(b) |= sample>>8;//((sample >> 8) & 0xff);
        }*/
    }
